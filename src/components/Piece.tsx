...
// Map each piece type and color to its corresponding component
...
export const Piece: React.FC<PieceProps> = ({ piece, size }) => {
  // Safely access motion from window if available
  const motion = typeof window !== 'undefined' ? (window as any).motion : null;
  
  // Get the appropriate piece component based on type and color
  const PieceComponent = PIECE_COMPONENTS[piece.color][piece.type];
  
  if (!PieceComponent) {
    console.error(`No component found for ${piece.color}${piece.type}`);
    return null;
  }

  // If motion is not available, render a static version
  if (!motion) {
    return (
  ...
  // If motion is available, use motion components for animation
  const MotionDiv = motion.div;
  
  // Apply different animations based on piece type and state
  const animations = {
    // For kings in check, apply a pulsing animation
    kingInCheck: isKingInCheck ? {
      scale: [1, 1.1, 1],
      rotate: [0, -5, 5, -5, 5, 0],
    } : {},
    // Default animation for all pieces
    default: {
      opacity: 1, 
      scale: 1,
      y: 0
  ...
  return (
    <MotionDiv
      ...
      animate={piece.type === 'k' && isKingInCheck ? animations.kingInCheck : animations.default}
